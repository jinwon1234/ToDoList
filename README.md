...
현재까지 사용한 기술 : Spring, JPA, Spring Data JPA, Thymeleaf, QueryDsl
...
## 1️⃣ 구현한 기능
로그인, 회원가입, 회원정보변경(이름, 비밀번호, 프로필사진), ToDoForm 등록/수정/삭제, 검색기능
...
**4. JPA N+1 문제는 언제 발생하는가?**

   N+1문제는 연관관계 매핑시 일반적으로 모든 연관관계를 지연로딩으로 설정하기 때문에 발생하는 문제이다.
   이해하기 쉽게 정리하면 JPA에서 N+1문제는 DTO안에 연관관계를 맺은 다른 엔티티를 포함하지 않으면 된다.

   ```java
      public class MemberDto {
          private Long id;
          private String name;
          private ToDoForm todoForm; // 엔티티 포함
      }
   ```
   실제로 이런 코드가 포함되지는 않았지만 만약 이런 코드가 존재한다면 N+1문제가 생길 수 있다. -> 페치 조인(혹은 @EntityGraph)을 통해 한꺼번에 조회해 N+1문제를 방지할 수 있다.
...
<img src="https://github.com/user-attachments/assets/9d5d2c47-0528-4b8f-950a-2508f50ea90a" width="300" height="400">
